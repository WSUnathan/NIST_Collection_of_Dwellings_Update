# -*- coding: utf-8 -*-
#%% import needed modules and user defines directory path
...
output_file("YearBuiltRemoved_Percentage_coverage_" + dataset + ".html")
...
#%%
#def apartment_WA(apartment, value, weight):
#    val = apartment[value]
#    wt = apartment[weight]
#    return (val * wt).sum() / wt.sum()

#def attached_WA(attached, value, weight):
#    val = attached[value]
#    wt = attached[weight]
#    return (val * wt).sum() / wt.sum()

#def detached_WA(detached, value, weight):
#    val = detached[value]
#    wt = detached[weight]
#    return (val * wt).sum() / wt.sum()

#def mobile_home_WA(mobile_home, value, weight):
#    val = mobile_home[value]
#    wt = mobile_home[weight]
#    return (val * wt).sum() / wt.sum()

#%%
#mobile_home_characteristic1 = mobile_home.groupby(['home_definition']).agg({'home_definition_year':['min'],'floor_area': ['min'],'year_built':['min'],
#    'forced-air':['min'],'home_definition':['count'],'weight':['sum'],'central_ac': ['count'],
#    'individual_ac': ['count'],'no_ac': ['count']})
#%%
#mobile_home_characteristic2 = mobile_home.groupby(['home_definition']).apply(mobile_home_WA,'#_of_bedrooms', 'weight')